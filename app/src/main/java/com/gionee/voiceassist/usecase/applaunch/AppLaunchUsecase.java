package com.gionee.voiceassist.usecase.applaunch;
...
public class AppLaunchUsecase extends BaseUsecase {
    public static final String TAG = AppLaunchUsecase.class.getSimpleName();
...
//    private RecordController recordController;
    ...
    public AppLaunchUsecase() {
        mAppCtx = GnVoiceAssistApplication.getInstance().getApplicationContext();
    }

    @Override
    public void handleDirective(DirectiveEntity payload) {
        AppLaunchDirectiveEntity applaunchPayload = (AppLaunchDirectiveEntity) payload;
        AppLaunchDirectiveEntity.LaunchType launchType = applaunchPayload.getLaunchType();
        switch (launchType) {
            case BY_NAME:
                fireByName(applaunchPayload);
                break;
            case BY_DEEPLINK:
    ...
    public void fireByName(AppLaunchDirectiveEntity payload) {
        boolean hasAppName = !TextUtils.isEmpty(payload.getAppName());
        boolean hasPkgName = !TextUtils.isEmpty(payload.getPackageName());
        if (hasPkgName) {
            launchAppByPackageName(payload.getPackageName());
        } else if (hasAppName) {
            launchAppByName(payload.getAppName());
        } else {
            LogUtil.e(TAG, "AppLaunch结果既没有包名也没有应用名，无法打开应用");
        }
    }

    public void fireByDeeplink(AppLaunchDirectiveEntity payload) {
        boolean hasDeeplink = TextUtils.isEmpty(payload.getDeeplink());
        if (hasDeeplink) {
            launchAppByDeepLink(payload.getDeeplink());
        } else {
            LogUtil.e(TAG, "AppLaunch结果没有指定uri，无法打开应用");
        }
    }
                ...
                for (int idx = 0; idx < pkgList.size(); idx++) {
                    ...
                    if (appName.contains(appNameTmp) || appNameTmp.contains(appName)) {
                        ...
                        if(isInstalledSuccess) {
                            playAndRenderText("正在打开" + appName);
                            break;
                        }
                    }
                }
    ...
    private void disappearSelectButton() {
        simpleAppItem.dismissButtonBar();
    }

    @RequiresApi(api = Build.VERSION_CODES.JELLY_BEAN)
    private void launchAppMarketAndDownload() {
    ...
    private void confirmDownload() {
        ...
        playAndRenderText(text, UTTER_CONFIRM_DOWNLOAD, new TtsCallback() {
            @Override
