...
import com.gionee.voiceassist.directiveListener.customcmd.CustomCmdDirectiveListener;
...
public class DirectiveListenerManager {
    ...
    private CustomCmdDirectiveListener customCmdDirectiveListener;
...
//        DcsSDK.getInstance().getAudioRecord().registerRecordListener(voiceInputVolumeListener);
        ...
        SdkController.getInstance().getSdkInternalApi().addDirectiveReceivedListener(new IDirectiveReceivedListener() {
            @Override
        ...
        //初始化错误回调接口
        SdkController.getInstance().getSdkInternalApi().addErrorListener(new IErrorListener() {
            @Override
        ...
        //初始化事件监听回调接口
        SdkController.getInstance().getSdkInternalApi().addRequestBodySentListener(new IDcsRequestBodySentListener() {
            @Override
        ...
        //初始化对话回调接口
        SdkController.getInstance().getSdkInstance().getVoiceRequest().addDialogStateListener(asrVoiceInputListener);
        SdkController.getInstance().getSdkInternalApi().setLocationHandler(locationHandler);
        ...
        ((CustomUserInteractionDeviceModule)getDeviceModule("ai.dueros.device_interface.extensions.custom_user_interaction")).setCustomUserInteractionListener(cuiDirectiveListener);
...
//        DcsSDK.getInstance().getAsr().registOfflineListener(null);
        ...
        ((CustomUserInteractionDeviceModule)getDeviceModule("ai.dueros.device_interface.extensions.custom_user_interaction")).setCustomUserInteractionListener(null);
    ...
    public void initDirectiveListener() {
        ...
        // 除 kookong 外其他的自定义指令回调 listener
        customCmdDirectiveListener = new CustomCmdDirectiveListener(baseFunctionManager);
    }
    ...
    public void onDestroy(){
        ...
        if(customCmdDirectiveListener != null) {
            customCmdDirectiveListener.onDestroy();
        }
    ...
    private BaseDeviceModule getDeviceModule(String namespace) {
        return SdkController.getInstance().getSdkInternalApi().getDeviceModule(namespace);
    }
}
